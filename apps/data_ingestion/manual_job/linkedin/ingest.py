...
def ingest_csv(conn, file_path, table_name):
    ...
    try:
        ...
        # For now, let's try UTF-8 as it's the most common
...
def main():
    ...
    if not os.path.exists(DATA_PATH):
        ...
        print(f"Please create it and put your LinkedIn CSV exports there.")
        sys.exit(1)
    ...
    for file_path in tqdm(csv_files, desc="Ingesting Files"):
        ...
        # Use filename as table name, santized
        table_name = os.path.splitext(filename)[0].lower().replace(' ', '_').replace('-', '_').replace('(', '').replace(')', '')
