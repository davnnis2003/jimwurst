    ...
    if uploaded_file is not None:
        ...
        if st.button("Send file details to agent", key="confirm_file_source"):
            if source_choice == "Others (Please specify)" and not other_source:
                ...
                # For LinkedIn exports, call the dedicated ingestion script
                if source_choice in ["Linkedin (Simple Export)", "Linkedin (Full Export)"]:
                    try:
                        from apps.data_ingestion.manual_job.linkedin.ingest import (
                            ingest_uploaded_linkedin_file,
                        )

                        mode = "basic" if source_choice == "Linkedin (Simple Export)" else "complete"
                        status_msg = ingest_uploaded_linkedin_file(file_path, mode)
                        st.info(status_msg)

                        # Seed a follow-up prompt for the agent
                        next_prompt = (
                            f"The LinkedIn {mode} export file '{os.path.basename(file_path)}' "
                            f"has been ingested into the data warehouse. "
                            f"Please summarize what LinkedIn content and engagement data is now available "
                            f"and suggest useful insights or queries I should run."
                        )
                    except Exception as e:
                        st.error(f"Failed to process LinkedIn file via ingestion script: {str(e)}")
                else:
                    # Default behavior for non-LinkedIn sources (generic ingestion via agent tools)
                    next_prompt = (
